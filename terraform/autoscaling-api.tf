resource "aws_launch_configuration" "api-launchconfig" {
  ...
  image_id        = var.API_INSTANCE_AMI // image created by Packer
  ...
  lifecycle {
    //replacement object is created first, and the prior object is destroyed after the replacement is created.
